...
async function fetchMovies(title, signal) {
  console.log(title);
  const res = await axios
    .get(`http://www.omdbapi.com/?apikey=${KEY}&s=${title}`, { signal: signal })
...
export default function App() {
  ...
  const { movies, isLoading, error } = useMovies(query, function () {
    handleCloseMovie();
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  function exist() {
    return watched.some((w) => w.imdbID === selectedId);
  }
  function handleAdd() {
    ...
    if (watched.some((w) => w.imdbID === selectedId)) {
      return;
    }
